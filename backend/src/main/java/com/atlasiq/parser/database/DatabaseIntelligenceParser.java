...
@ApplicationScoped public class DatabaseIntelligenceParser {
 ...
 private boolean safe(Path r,Path p){Path rel=r.relativize(p);for(Path x:rel)if(X.contains(x.toString()))return false;return Files.isRegularFile(p,LinkOption.NOFOLLOW_LINKS)&&!Files.isSymbolicLink(p)&&smallEnough(p);}
 private boolean smallEnough(Path p){try{return Files.size(p)<2_000_000;}catch(IOException e){return false;}}
